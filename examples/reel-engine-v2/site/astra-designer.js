/* ============================================================================
   astra-designer: a scroll-driven interaction runtime
   ----------------------------------------------------------------------------
...
(function (global) {
  ...
  function mount(root, opts) {
    ...
    function layout() {
      ...
      acts.forEach(function (a) {
        ...
        if (pos !== 'sticky' && pos !== '-webkit-sticky') {
          console.warn('[astra-designer] act "' + (a.el.id || a.device) + '" will not pin: its stage computes ' +
      ...
      worlds.forEach(function (W) {
        ...
        W.checked = true;
        if (!W.stage) {
          console.warn('[astra-designer] worldflight has no [data-sc-world] stage; nothing will fly.', W.el);
          return;
        }
        if (!W.spacer) {
          console.warn('[astra-designer] worldflight has no [data-sc-spacer]; the page has no scroll track.', W.el);
        }
        ...
        if (wp !== 'fixed') {
          console.warn('[astra-designer] worldflight stage computes position:' + wp + ', not fixed. ' +
    ...
    // definition. behavior:'instant' on purpose: astra-designer.css sets
    ...
    global.AstraDesigner.instances.push(api);
    return api;
  }
  ...
  global.AstraDesigner = { mount: mount, reduce: reduce, instances: [] };
})(window);
