...
For the production standard’s functional, fallback, and deployment acceptance
...
**Generation accounting.** Record provider job receipts for this build. Shared account balance differences do not establish per-build cost.

## Failures worth knowing about

These are known failure patterns to check during visual verification.
