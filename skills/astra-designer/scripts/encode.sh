#!/usr/bin/env bash
# astra-designer: encode a clip for scrubbing, not for playback.
#
...
#   ASTRA_DESIGNER_CRF=23 ./encode.sh in.mp4 out.mp4    or the env var
#
...
pick_ffmpeg() {
  ...
  echo "ERROR: no full ffmpeg build found. Set ASTRA_DESIGNER_FFMPEG to one." >&2
  return 1
}
FFMPEG="$(pick_ffmpeg)"
