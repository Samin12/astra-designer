#!/usr/bin/env node
/**
 * Resolve the astra-designer workspace: the one directory that holds this user's
 ...
 *
 *   1. ASTRA_DESIGNER_HOME              env var, absolute or relative to cwd
 *   2. .astra-designer.json             nearest one walking up from cwd,
 *                                    { "workspace": "some/path" } relative to
 *                                    the file that declares it
 *   3. <project root>/astra-designer    project root = nearest ancestor with .git,
 *                                    otherwise cwd
 *
 * USAGE
...
export function resolveWorkspace(cwd = process.cwd()) {
  if (process.env.ASTRA_DESIGNER_HOME) {
    return { root: path.resolve(cwd, process.env.ASTRA_DESIGNER_HOME), via: "ASTRA_DESIGNER_HOME" };
  }

  const cfg = findUp(cwd, (d) => {
    const p = path.join(d, ".astra-designer.json");
  ...
  if (cfg) {
    ...
    try {
      ...
      throw new Error(`.astra-designer.json is not valid JSON: ${cfg.hit}\n  ${e.message}`);
    }
    if (parsed.workspace) {
  ...
  return { root: path.join(base, "astra-designer"), via: git ? "project root (.git)" : "cwd" };
}
